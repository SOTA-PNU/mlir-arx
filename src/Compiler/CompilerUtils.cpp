...
extern std::unique_ptr<Partition> partition_;
...
//
//void loadPartitionPlan(std::string partitionPlan, PartitionPlanInfo partitionPlanInfo) {
//
//}
//
//void loadDeviceConfig(std::string deviceConfig, std::map<std::string, std::unique_ptr<std::map<std::string, std::string>>>& deviceConfigMap) {
//  if(deviceConfig.empty()) {
//    std::cout << "No deviceConfig file: " << std::endl;
//    return;
//  }
//  std::cout << "deviceConfig file: " << deviceConfig << std::endl;
//
//  try {
...
////        std::cout << "Backend Name: " << node["backendName"].as<std::string>() << std::endl;
////        std::cout << "Device ID: " << node["deviceID"].as<int>() << std::endl;
//
//        if (node["isDefaultDevice"]) {
//          std::cout << "Is Default Device: " << (node["isDefaultDevice"].as<bool>() ? "true" : "false") << std::endl;
//        }
//
////        std::cout << "Supported Ops: " << node["supportedOps"].as<std::string>() << std::endl;
////        std::cout << "Emit Type: " << node["emitType"].as<std::string>() << std::endl;
////        std::cout << "----------------------" << std::endl;
//
//        std::string deviceName = node["name"].as<std::string>();
//
//        if (deviceConfigMap.find(deviceName) == deviceConfigMap.end()) {
//          //itemMap = std::make_unique<std::map<std::string, std::string>>();
//          deviceConfigMap[deviceName] = std::make_unique<std::map<std::string, std::string>>();
//        }
//        std::map<std::string, std::string>& itemMap = *deviceConfigMap[deviceName];
//
...
//
//        if (node["isDefaultDevice"]) {
//          itemMap["isDefaultDevice"] = (node["isDefaultDevice"].as<bool>() ? "true" : "false");
//        } else {
...
int processInputFileWithPartition(StringRef inputFilename, std::vector<mlir::MLIRContext*> &contextList,
    std::vector<mlir::OwningOpRef<ModuleOp>> &partModules, std::string *errorMessage, std::unique_ptr<Partition> partition) {
  // Decide if the input file is an ONNX model (either ONNX protobuf, ONNX text,
  // or JSON) or a model specified in MLIR.
  // The extension of the file is the decider.
  bool inputIsSTDIN = (inputFilename == "-");
  ...
  if (inputIsSTDIN || inputIsONNX || inputIsONNXText || inputIsJSON) {
    ...
    options.functionsToDecompose.insert(options.functionsToDecompose.end(),
        functionsToDecompose.begin(), functionsToDecompose.end());
    return ImportFrontendModelFileWithPartition(
        inputFilename, contextList, partModules, errorMessage, options);
  } else if (inputIsMLIR) {
//    loadMLIR(inputFilename.str(), context, module);
    *errorMessage = "Not supported yet with partitioning functions.";
...
EmissionTargetType getEmitType(std::string emissionTarget) {
  if(emissionTarget == "Obj"){
    return EmitObj;
  } else if(emissionTarget == "Lib") {
    return EmitLib;
  } else if(emissionTarget == "JNI") {
    return EmitJNI;
  } else if(emissionTarget == "MLIR") {
    return EmitMLIR;
  } else if (emissionTarget == "ONNXBasic") {
    return EmitONNXBasic;
  } else if (emissionTarget == "ONNXIR") {
    return EmitONNXIR;
  }
  return EmitObj;
}
...
//  std::cout << "(compileModule) " << module.get(). << std::endl;
