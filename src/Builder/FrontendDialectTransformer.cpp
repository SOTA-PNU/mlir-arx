...
std::unique_ptr<Partition> partition_ = nullptr;
namespace {
...
//    std::cout << "model function size = " << model.functions_size() << std::endl;
  ...
  ModuleOp ImportONNXModel(
    ...
    opset_map_ = GetOpsetImportsFromProto(model); // Which opsets to use.
    in_model_functions_ = GetModelLocalFunctions(model);
    importGraph(model.graph(), graphName);
    if (options_.verboseOutput) {
      llvm::outs()
          << "The ONNX model has " << num_of_parameters_
          << " elements in its initializers. This value would be close to and "
...
//    std::cout << "== [ConvertOnnxType] ==" << std::endl;
...
//    std::cout << "== [convertOnnxAttributeProtoToMlirNamedAttribute] ==" << std::endl;
    Attribute mlirAttr;
    switch (attr.type()) {
...
//      std::cout << "========================== " << std::endl;
//      std::cout << "onnx node = " << item.name() << std::endl;
//      std::cout << "optype = " << item.op_type() << std::endl;
//
//      std::cout << "input 0 = " << item.input(0) << std::endl;
//      std::cout << "out size = " << item.output_size() << std::endl;
//      std::cout << "out 0 = " << item.output(0) << std::endl;
//      std::cout << "========================== " << std::endl;
...
//    std::cout << "== [ImportOutputTensor] ==" << std::endl;
...
//    std::cout << "== [importGraph 2] ==" << std::endl;
  ...
  func::FuncOp importGraph(const onnx::GraphProto &graph, std::string graphName) {
//    std::cout << "== [importGraph 2] ==" << std::endl;
//    const std::string &name = std::string("main_graph") + "_" + graphName;
    const std::string &name = graphName;
    std::cout << "graph name = " << name << std::endl;
...
void setDimByName(const std::string& name, const onnx::GraphProto& graph, onnx::ValueInfoProto* value) {
  ...
  for(auto vi: graph.value_info()) {
    if(!vi.name().compare(name) && vi.has_type()) {
      auto* tensorShapeProto = tensor_type->mutable_shape();
      for (auto dim: vi.type().tensor_type().shape().dim()) {
        auto* newDim = tensorShapeProto->add_dim();
        newDim->set_dim_value(dim.dim_value());
      }
      tensor_type->set_elem_type(vi.mutable_type()->mutable_tensor_type()->elem_type());
    }
  }
}

void generateDefaultPartitionPlan(const onnx::ModelProto &originalModel) {
...
//        partition = "p1";
  ...
  if (!planFile.is_open()) {
    std::cerr << "Failed to create PartitionPlan.yaml." << std::endl;
    return;
  }
  planFile << "---" << std::endl;
  planFile << "- name: default" << std::endl;
  planFile << "  numPartitions: " << "1" << std::endl;
  planFile << "  backendNames: " << "CPU" << std::endl;
  planFile << "  emitNames: " << "Obj" << std::endl;
  planFile << "  partitionNames: " << "p0" << std::endl;
  planFile << "  nodeToPartition: " << nodeNames << std::endl;
  planFile << "..." << std::endl;
...
const onnx::ValueInfoProto* getInputValueProto(std::string name, const onnx::ModelProto& model) {

  for(int i = 0; i < model.graph().input_size(); i++) {
    if(!name.compare(model.graph().input(i).name())) {
...
const onnx::ValueInfoProto* getOutputValueProto(std::string name, const onnx::ModelProto& model) {

  for(int i = 0; i < model.graph().output_size(); i++) {
    if(!name.compare(model.graph().output(i).name())) {
...
void getPrevNode(onnx::NodeProto* node, std::vector<onnx::NodeProto*>* prevNodes, const onnx::ModelProto* model) {
  for(int i = 0; i < model->graph().node_size(); i++) {
    ...
    if(snode.name().compare(node->name())) {
...
void getNextNode(onnx::NodeProto* node, std::vector<onnx::NodeProto*>* nextNodes, onnx::ModelProto* model) {
  for(int i = 0; i < model->graph().node_size(); i++) {
    auto snode = model->graph().node(i);

    if(snode.name().compare(node->name())) {
      for(int k = 0; k < node->output_size(); k++) {
        std::string outName = node->output(k); //next input

        for(int sk = 0; sk < snode.input_size(); sk++) {
          std::string sinName = snode.input(sk); //current output
          if(!sinName.compare(outName)) {
            nextNodes->push_back(&snode);
          }
        }
      }
    }
...
bool isInputExist(const std::string& inName, onnx::ModelProto* model) {
  for(auto input: model->graph().input()) {
      if (!inName.compare(input.name()))
        return true;
  }
  return false;
}
...
std::unique_ptr<onnx::ModelProto> makeSubgraph(size_t seq, std::string partitionName,
  ...
  std::unique_ptr<onnx::ModelProto> submodel = std::make_unique<onnx::ModelProto>();
  submodel.get()->set_producer_name(originalModel.producer_name());
  submodel.get()->set_ir_version(originalModel.ir_version());
  for(auto op: originalModel.opset_import()) {
    *(submodel.get()->add_opset_import()) = op;
  }
...
//        copyNodeInOut(originalModel, *(submodel.get()), tnode);
  ...
  std::vector<onnx::NodeProto*> tmpList;
...
//        std::cout << "gout name = " << gout->name() << std::endl;
  ...
  for(auto totalin: subGraphInputList) {
      ...
      if(!found) {
        auto newin = submodel->mutable_graph()->add_input();
        newin->CopyFrom(*totalin);
      }
  }
...
//  onnx::ModelProto* prevModel = nullptr;
  for (size_t p = 0; p < partNames.size(); p++) {
    ...
    std::ofstream submodel_stream(
        partition_->getPartitionPlanInfo().modelName_ + "_" +
            submodel->mutable_graph()->name() + ".onnx",
        std::ios::binary);
...
//  } else {
//    //generate PartitionPlan.yaml
//    generateDefaultPartitionPlan(model);
//    return;
//  }
...
//  std::cout << std::endl << "=== [ImportFrontendModel start] ===" << std::endl;
 ...
 if(partition_->getPartitionPlanInfo().partitionNames_.size() == 0) {
   generateDefaultPartitionPlan(model);
   exit(0);
 }

  int partNum = partition_->size();
  if(partNum == 0) {
    partNum = 1;
  }

  std::vector<detail::FrontendGenImpl> genImplList;
  for(int i = 0; i < partNum; i++){
    detail::FrontendGenImpl myONNXGen(*contextList.at(i));
    genImplList.push_back(myONNXGen);
  }

  if(partition_->size() == 0) {
    auto module = (genImplList.at(0)).ImportONNXModel(model, options);
    ...
    for (size_t i = 0; i < submodelList.size(); i++) {
      std::string name = std::string("p") + std::to_string(i);
      onnx::ModelProto *submodel = std::move((submodelList.at(i).second)).get();
      auto module =
          (genImplList.at(i)).ImportONNXModel(*submodel, options, name);
      partModules.push_back(module);
    }
  }
}
...
bool ImportFrontendModelInternalWithPartition(onnx::ModelProto &model, std::vector<mlir::MLIRContext*> &contextList,
  ...
  // onnx-mlir handles the CustomOp in a different way. It assumes the common
  // pattern that the element type of the output is the same
  // as the first input. And later shape inference will use the
  // shape-inference-pattern attribute to perform shape inference on CustomOp.
  // The type assumption in the Importer may be incorrect and cause
  // trouble.
  // ToFix: the shape-inference-pattern should be added and used in Importer.
  ...
  if (options.invokeOnnxVersionConverter &&
    ...
    if (options.useOnnxModelTypes)
      onnx::shape_inference::InferShapes(convertModel);
    ImportFrontendModelWithPartition(convertModel, contextList, partModules, options);
  } else {
    if (options.useOnnxModelTypes)
      onnx::shape_inference::InferShapes(model);
    ImportFrontendModelWithPartition(model, contextList, partModules, options);
  }
  return true;
}
...
int ImportFrontendModelFileWithPartition(StringRef model_fname, std::vector<mlir::MLIRContext*> &contextList,
    std::vector<OwningOpRef<ModuleOp>> &partModules, std::string *errorMessage,
    ImportOptions options) {
  onnx::ModelProto model;
  if (model_fname.endswith(".onnxtext")) {
    std::string text;
