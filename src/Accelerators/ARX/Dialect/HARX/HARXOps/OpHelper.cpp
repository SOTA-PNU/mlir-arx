...
//===-------- OpHelper.cpp - ARX HARX Helper Functions ------------------===//
//
...
bool oneIsOfLayout(Type t1, Type t2,
  ...
  if (auto rtp1 = llvm::dyn_cast<RankedTensorType>(t1)) {
    if (onnx_mlir::harx::getZTensorLayout(rtp1) == layout)
      return true;
    // t1 is not of `layout`, check t2.
    if (auto rtp2 = llvm::dyn_cast<RankedTensorType>(t2)) {
      return (onnx_mlir::harx::getZTensorLayout(rtp2) == layout);
    }
    // t2 is unranked.
  }
  // t1 is unranked.
