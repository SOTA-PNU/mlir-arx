...
#include "src/Accelerators/ARX/Dialect/HARX/HARXOps/Unstick/ONNXHARXUnstick.inc"
...
LogicalResult HARXUnstickOpShapeHelper::computeShape() {
  HARXUnstickOp::Adaptor operandAdaptor(operands);
...
LogicalResult HARXUnstickOp::inferShapes(
    std::function<void(Region &)> doShapeInference) {
  if (!hasRankedType(getIn()))
    return success();

  HARXUnstickOpShapeHelper shapeHelper(getOperation());
  Type elementType =
...
void HARXUnstickOp::getCanonicalizationPatterns(
