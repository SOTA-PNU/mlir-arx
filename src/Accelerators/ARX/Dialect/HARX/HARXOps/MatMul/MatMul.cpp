...
LogicalResult HARXMatMulOpShapeHelper::computeShape() {
  // HARXMatMulOp::Adaptor operandAdaptor(operands);
...
LogicalResult HARXMatMulOp::inferShapes(
    std::function<void(Region &)> doShapeInference) {
  // if (!hasRankedType(getX()) || !hasRankedType(getY()))
  //   return success();

  // HARXMatMulOpShapeHelper shapeHelper(getOperation());
...
// LogicalResult HARXMatMulOp::verify() {
  // HARXMatMulOpAdaptor operandAdaptor(*this);
  // // Get operands.
  // Value X = operandAdaptor.getX();
  // Value Y = operandAdaptor.getY();
