...
LogicalResult HARXPoolingOpShapeHelper<OP>::computeShape() {
...
//===----------------------------------------------------------------------===//
// HARX Shape Helper template instantiation
// Keep template instantiation at the end of the file.
//===----------------------------------------------------------------------===//

template struct HARXPoolingOpShapeHelper<HARXMaxPool2DOp>;
template struct HARXPoolingOpShapeHelper<HARXAvgPool2DOp>;

//===----------------------------------------------------------------------===//
// MaxPool2DOp
//===----------------------------------------------------------------------===//

LogicalResult HARXMaxPool2DOp::inferShapes(
  ...
  HARXPoolingOpShapeHelper<HARXMaxPool2DOp> shapeHelper(getOperation());
...
LogicalResult HARXAvgPool2DOp::inferShapes(
    std::function<void(Region &)> doShapeInference) {
  if (!hasRankedType(getInput()))
    return success();
  ...
  RankedTensorType inputType =
      mlir::cast<RankedTensorType>(getInput().getType());
  HARXPoolingOpShapeHelper<HARXAvgPool2DOp> shapeHelper(getOperation());
  return shapeHelper.computeShapeAndUpdateType(
      inputType.getElementType(), inputType.getEncoding());
}
