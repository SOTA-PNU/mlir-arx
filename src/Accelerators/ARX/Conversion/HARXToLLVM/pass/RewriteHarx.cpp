
#include "mlir/Conversion/LLVMCommon/Pattern.h"

#include "src/Accelerators/ARX/Conversion/HARXToLLVM/HARXToLLVM.hpp"
#include "src/Accelerators/ARX/Dialect/HARX/HARXOps.hpp"
#include "src/Accelerators/ARX/Support/LayoutHelper.hpp"
#include "src/Conversion/KrnlToLLVM/KrnlToLLVMHelper.hpp"
#include "src/Dialect/Mlir/DialectBuilder.hpp"
#include "src/Dialect/ONNX/ONNXOps.hpp"

#include "mlir/Dialect/Bufferization/IR/Bufferization.h"
#include "mlir/Dialect/EmitC/IR/EmitC.h"

#include "mlir/Target/LLVMIR/TypeToLLVM.h"   // DataLayout, TypeToLLVM
#include "src/Accelerators/ARX/Conversion/HARXToLLVM/pass/HarxToEmitC.h"

#include <random>

using namespace mlir;
using namespace onnx_mlir;

namespace onnx_mlir {
namespace larx {

LogicalResult ConstantToEmitCPattern::matchAndRewrite(Operation *ops, ArrayRef<Value> operands, ConversionPatternRewriter &rewriter) const { 
  auto module = ops->getParentOfType<mlir::ModuleOp>();
  // auto ctx = ops->getContext();
  auto op = mlir::cast<harx::HARXConstantOp>(ops);

  auto dataValue = op.getValueAttr();
  auto elemDataTy = dataValue.getShapedType();
  auto dataTy = mlir::emitc::ArrayType::get(elemDataTy.getShape(), elemDataTy.getElementType());
  auto harxName = op.getOperation()->getAttrOfType<mlir::StringAttr>("harx_name");

  // SmallVector<Attribute> flat;
  // flat.reserve(dataValue.getNumElements());

  // if (auto intElems = dataValue.dyn_cast<DenseIntElementsAttr>()) {
  //   Type elemTy = intElems.getType().getElementType();
  //   for (APInt v : intElems.getValues<APInt>())
  //     flat.push_back(rewriter.getIntegerAttr(elemTy, v));
  // }
  // // Float case
  // else if (auto fpElems = dataValue.dyn_cast<DenseFPElementsAttr>()) {
  //   Type elemTy = fpElems.getType().getElementType();
  //   for (APFloat v : fpElems.getValues<APFloat>())
  //     flat.push_back(rewriter.getFloatAttr(elemTy, v));
  // }else {
  //   llvm::errs() << "Unsupported constant type: " << dataValue.getType() << "\n";
  //   return rewriter.notifyMatchFailure(ops, "Unsupported constant type");
  // }
  // auto arrayAttr = rewriter.getArrayAttr(flat);

  llvm::dbgs() << dataValue << "\n";

  if (!module.lookupSymbol<mlir::emitc::GlobalOp>(harxName)) {
    OpBuilder::InsertionGuard g(rewriter);
    rewriter.setInsertionPointToStart(module.getBody());
    rewriter.create<mlir::emitc::GlobalOp>(
      module.getLoc(),
      /* sym_name */      harxName,
      /* type */          dataTy,
      /* initializer */   dataValue,
      /* extern? */       false,
      /* static? */       true,
      /* const? */        true);
  }

  auto symRef = mlir::FlatSymbolRefAttr::get(rewriter.getContext(), harxName);
  auto getGlobal = rewriter.create<mlir::emitc::GetGlobalOp>(
      op.getLoc(),
      dataTy,
      symRef);

  rewriter.replaceOp(op, getGlobal.getResult());

  return success();
}



LogicalResult QuantizeToEmitCPattern::matchAndRewrite(harx::HARXQuantizationOp op,PatternRewriter &rewriter) const {
  // auto retTy = rewriter.getI32Type();
  // op.getInput()
  auto ui8Ty = rewriter.getIntegerType(8, false);
  auto fp32Ty = rewriter.getF32Type();

  auto numElements = op.getInput().getType().getNumElements();
  auto numElemTy = rewriter.getIntegerType(32, false);
  auto numElemVal = rewriter.getIntegerAttr(numElemTy, numElements);
  auto numElemAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), numElemTy, numElemVal).getResult();

  auto scaleVal = op.getScaleAttr();
  auto scaleTy = emitc::ArrayType::get({1}, fp32Ty);
  SmallVector<APFloat, 1> scaleVec;
  scaleVec.reserve(1); scaleVec.push_back(scaleVal.getValue());
  DenseFPElementsAttr scaleDenseAttr = DenseFPElementsAttr::get(scaleTy, scaleVec);
  auto scaleAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), scaleTy, scaleDenseAttr).getResult();

  auto offsetVal = op.getOffsetAttr();
  auto offsetTy = emitc::ArrayType::get({1}, ui8Ty);
  SmallVector<APInt, 1> offsetVec;
  offsetVec.reserve(1); offsetVec.push_back(offsetVal.getValue());
  DenseIntElementsAttr offsetDenseAttr = DenseIntElementsAttr::get(offsetTy, offsetVec);
  auto offsetAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), offsetTy, offsetDenseAttr).getResult();
  
  auto module = op.getOperation()->getParentOfType<mlir::ModuleOp>();
  auto quantFn = module.lookupSymbol<emitc::FuncOp>("quantize");
  
  auto retTy = op.getResult().getType();

  auto harxName = op.getOperation()->getAttrOfType<mlir::StringAttr>("harx_name");
  auto outputTy = emitc::ArrayType::get(retTy.getShape(), ui8Ty);
  SmallVector<uint8_t> outputVec;
  outputVec.reserve(numElements);
  for (int i = 0; i < (int)numElements; ++i) {
    outputVec.push_back(0); //   APInt(8, 0, false)

    // outputVec.push_back(0);
  }
  
  if (!module.lookupSymbol<mlir::emitc::GlobalOp>(harxName)) {
    OpBuilder::InsertionGuard g(rewriter);
    DenseIntElementsAttr outputDenseAttr = DenseIntElementsAttr::get(outputTy, outputVec);
    rewriter.setInsertionPointToStart(module.getBody());
    rewriter.create<mlir::emitc::GlobalOp>(module.getLoc(),
      /* sym_name */      harxName,
      /* type */          outputTy, Attribute(), false, true, false);
  }
    
    // // DenseIntElementsAttr outputDenseAttr = DenseIntElementsAttr::get(outputTy, outputVec);
    
    // OpBuilder::InsertionGuard g(rewriter);
    // rewriter.setInsertionPointToStart(module.getBody());
    // rewriter.create<mlir::emitc::GlobalOp>(module.getLoc(), TypeRange{}, harxName, outputTy, outputDenseAttr, false, true, false);
  // }
  
  auto symRef = mlir::FlatSymbolRefAttr::get(rewriter.getContext(), harxName);
  auto getGlobal = rewriter.create<mlir::emitc::GetGlobalOp>(op.getLoc(), outputTy, symRef).getResult();
  // auto getGlobal = rewriter.create<mlir::emitc::LiteralOp>(op.getLoc(), outputTy, harxName).getResult();

  auto fnInputTy = op.getInput().getType();
  auto inputTy = emitc::ArrayType::get(fnInputTy.getShape(), fnInputTy.getElementType());
  auto inAttr = op.getInput();
  // auto inAttr = rewriter.create<UnrealizedConversionCastOp>(op.getLoc(), inputTy, op.getInput()).getResult(0);

  auto ui8Ptr = emitc::PointerType::get(ui8Ty);
  auto fp32Ptr = emitc::PointerType::get(fp32Ty);
  
  auto ptrOp = rewriter.getStringAttr("&");

  auto shape = inputTy.getShape();
  SmallVector<Value> shapeVec;
  auto dim = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), rewriter.getI32Type(), rewriter.getI32IntegerAttr(0)).getResult();
  for (int i = 0; i < (int)shape.size(); ++i) {
    shapeVec.push_back(dim);
  }
  
  auto eui8Ty = mlir::emitc::LValueType::get(ui8Ty);
  auto efp32Ty = mlir::emitc::LValueType::get(fp32Ty);

  auto getGlobalSubAttr = rewriter.create<mlir::emitc::SubscriptOp>(op.getLoc(), eui8Ty, getGlobal, ValueRange(shapeVec)).getResult();
  auto getGlobalSubPtrAttr = rewriter.create<mlir::emitc::ApplyOp>(op.getLoc(), ui8Ptr, ptrOp, getGlobalSubAttr).getResult();
  
  auto inSubAttr = rewriter.create<mlir::emitc::SubscriptOp>(op.getLoc(), efp32Ty, inAttr, ValueRange(shapeVec)).getResult();
  auto inSubPtrAttr = rewriter.create<mlir::emitc::ApplyOp>(op.getLoc(), fp32Ptr, ptrOp, inSubAttr).getResult();

  auto scaleSubAttr = rewriter.create<mlir::emitc::SubscriptOp>(op.getLoc(), efp32Ty, scaleAttr, ValueRange { dim }).getResult();
  auto scaleSubPtrAttr = rewriter.create<mlir::emitc::ApplyOp>(op.getLoc(), fp32Ptr, ptrOp, scaleSubAttr).getResult();

  auto offsetSubAttr = rewriter.create<mlir::emitc::SubscriptOp>(op.getLoc(), eui8Ty, offsetAttr, ValueRange { dim }).getResult();
  auto offsetSubPtrAttr = rewriter.create<mlir::emitc::ApplyOp>(op.getLoc(), ui8Ptr, ptrOp, offsetSubAttr).getResult();

  auto args = ValueRange({inSubPtrAttr, getGlobalSubPtrAttr, numElemAttr, scaleSubPtrAttr, offsetSubPtrAttr});
  rewriter.create<mlir::emitc::CallOp>(op.getLoc(), quantFn, args);

  // auto r = rewriter.create<UnrealizedConversionCastOp>(op.getLoc(), op.getOutput().getType(), getGlobal);

  rewriter.replaceOp(op, getGlobal);
  // rewriter.replaceOp(op, r.getResults());

  return mlir::success();
}

LogicalResult DequantizeToEmitCPattern::matchAndRewrite(harx::HARXDequantizationOp op, PatternRewriter &rewriter) const {
  auto numElements = op.getInput().getType().getNumElements();
  auto numElemTy = rewriter.getIntegerType(32, false);
  auto numElemVal = rewriter.getIntegerAttr(numElemTy, numElements);
  auto numElemAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), numElemTy, numElemVal).getResult();

  auto channelSizeNum = op.getChannelSize();
  auto channelSizeVal = op.getChannelSizeAttr();
  auto channelSizeTy = rewriter.getI32Type();
  auto channelSizeAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), channelSizeTy, channelSizeVal).getResult();

  auto scaleVal = op.getScale();
  auto scaleTy = emitc::ArrayType::get({channelSizeNum}, rewriter.getF32Type());
  SmallVector<float> scaleValues;
  scaleValues.reserve(channelSizeNum);
  for (int i = 0; i < (int)channelSizeNum; ++i) {
    scaleValues.push_back(scaleVal.convertToFloat());
  }
  DenseF32ArrayAttr scaleDenseAttr = DenseF32ArrayAttr::get(rewriter.getContext(), scaleValues);
  auto scaleAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), scaleTy, scaleDenseAttr).getResult();

  Type ui8Ty    = rewriter.getIntegerType(8, false);
  auto offsetVal = op.getOffset();
  auto offsetTy = emitc::ArrayType::get({channelSizeNum}, ui8Ty);
  SmallVector<uint8_t> offsetValues;
  offsetValues.reserve(channelSizeNum);
  for (int i = 0; i < (int)channelSizeNum; ++i) {
    offsetValues.push_back(offsetVal);
  }
  auto tensorTy = RankedTensorType::get({channelSizeNum}, ui8Ty);
  DenseIntElementsAttr offsetDenseAttr = DenseIntElementsAttr::get(tensorTy, offsetValues);
  auto offsetAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), offsetTy, offsetDenseAttr).getResult();

  auto useStrideVal = op.getUseStrideAttr();
  auto useStrideTy = rewriter.getIntegerType(8, false);
  auto useStrideAttr = rewriter.create<mlir::emitc::ConstantOp>(op.getLoc(), useStrideTy, useStrideVal).getResult();

  auto module = op.getOperation()->getParentOfType<mlir::ModuleOp>();
  auto dequantFn = module.lookupSymbol<emitc::FuncOp>("quantize");

  Value input = op.getInput();
  Value output = op.getOutput();

  rewriter.create<mlir::emitc::CallOp>(
                          op.getLoc(),
                          dequantFn,
                          ValueRange{input, output, numElemAttr, scaleAttr, offsetAttr, channelSizeAttr, useStrideAttr});
  
  rewriter.replaceOp(op, op.getInput());

  return mlir::success();
}
LogicalResult ConvolutionShiftToEmitCPattern::matchAndRewrite(harx::HARXConvolutionShiftOp op,PatternRewriter &rewriter) const {
  return mlir::success();
}
LogicalResult MaxPoolToEmitCPattern::matchAndRewrite(harx::HARXMaxPoolOp op,PatternRewriter &rewriter) const {
  return mlir::success();
}
LogicalResult ReShapeToEmitCPattern::matchAndRewrite(harx::HARXTransposeOp op,PatternRewriter &rewriter) const {
  return mlir::success();
}
LogicalResult FullyConnectedToEmitCPattern::matchAndRewrite(harx::HARXMatMulOp op,PatternRewriter &rewriter) const {
  return mlir::success();
}

} // namespace harx
} // namespace onnx_mlir
