...
//===--- RewriteONNXForHARX.cpp - Rewrite ONNX ops for HARX lowering ----===//
//
...
// `HARX.BatchNorm`, generating `ONNX.Add`, `ONNX.Sub`, `ONNX.Mul`, `ONNX.Div`,
// and `ONNX.Sqrt` to calculate inputs(`a` and `b`) for `HARX.BatchNorm`.
// `ONNXToHARXLoweringPass`(`--convert-onnx-to-harx`) is also able to generate
// the ONNX ops, but,they are lowered to HARX ops. So, constant
...
#include "src/Accelerators/ARX/Conversion/ONNXToHARX/RewriteONNXForHARX.hpp"
...
//
bool canInferencePadsForARXConv(ONNXConvOp op) {
...
ArrayAttr getPadsForARXConv(PatternRewriter &rewriter, Value ret) {
...
/// ARX. Given (NxK) * (K*M), the pattern considers dimensions N and/or M to
/// split, if N and/or M is greater than ARXGetMaxForDim (MDIS).
...
public:
  ...
  LogicalResult matchAndRewrite(
    ...
    // Expect N or M exceeds ARX limitation.
  ...
  static bool canBeRewritten(
    ...
    nExceeded = N > ARXGetMaxForDim(aRank - 2, aRank);
    mExceeded = M > ARXGetMaxForDim(bRank - 1, bRank);
...
//===----------------------------------------------------------------------===//
// Rewrite ONNX ops to HARX ops and ONNX ops for HARX.
//===----------------------------------------------------------------------===//

/// Include the patterns defined in the Declarative Rewrite framework.
#include "src/Accelerators/ARX/Conversion/ONNXToHARX/ONNXRewriteONNXForHARX.inc"
...
void getRewriteONNXForHARXDynamicallyLegal(
  ...
  // `ONNXBatchNormalizationInferenceModeOp` to `HARX.BatchNorm`,
  ...
  //
  // This is preferred for ARX because ARX BinaryOp does not support
  // broadcasting.
  ...
  // - no input is N-D (N > 3) but dimension size exceeds ARX limitation.
  //
      ...
      target, dimAnalysis, [](ONNXMatMulOp op, const DimAnalysis *dimAnalysis) {
        ...
        if (aRank > 3 && (aRank == bRank)) {
          ...
          return (!sameBatchDims) ||
                 onnxToHARXUnsupportedReport(op.getOperation(), message);
        }
  ...
  // - the ARX level is not compatible, or
...
struct RewriteONNXForHARXPass
    : public PassWrapper<RewriteONNXForHARXPass, OperationPass<ModuleOp>> {

  StringRef getArgument() const override { return "rewrite-onnx-for-harx"; }

  StringRef getDescription() const override {
    return "Rewrite ONNX ops for HARX.";
  }

  RewriteONNXForHARXPass() = default;
  void runOnOperation() final;
};

void RewriteONNXForHARXPass::runOnOperation() {
  ...
  // this lowering.
  target.addLegalDialect<ONNXDialect, harx::HARXDialect, func::FuncDialect>();
  onnx_mlir::getRewriteONNXForHARXDynamicallyLegal(&target, &dimAnalysis);

  // Single ONNX to HARX operation lowering.
  RewritePatternSet patterns(&getContext());
  onnx_mlir::getRewriteONNXForHARXPatterns(patterns, &dimAnalysis);
...
std::unique_ptr<Pass> createRewriteONNXForHARXPass() {
  return std::make_unique<RewriteONNXForHARXPass>();
}
