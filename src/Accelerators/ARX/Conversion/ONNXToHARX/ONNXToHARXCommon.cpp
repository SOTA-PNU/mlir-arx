...
//====----- ONNXToHARXCommon.cpp - Common functions to HARX lowering ----===//
//
...
// bool isEnableScalarBcastBinary() { return arxEnableScalarBcastBinary; }
...
/// ARXGetMaxForDim and the last chunk can be smaller.
ValueRange splitAlongAxis(
  ...
  // First splits have the same size of ARXGetMaxForDim.
  int64_t maxSize = ARXGetMaxForDim(axis, xRank);
...
int OnnxToHARXLoweringConfiguration::optReportARXUnsupportedOps =
    0; // 0: Compile option (--opt-report=ARXUnsupportedOps) not specified.
int OnnxToHARXLoweringConfiguration::reportOnARXUnsupportedOps =
    0; // 0: no reporting.
void configureOnnxToHARXLoweringPass(bool optReportARXUnsupportedOps) {
  OnnxToHARXLoweringConfiguration::optReportARXUnsupportedOps =
      optReportARXUnsupportedOps;
}
