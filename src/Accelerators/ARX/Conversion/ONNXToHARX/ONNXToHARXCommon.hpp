...
//===---------- ONNXToHARXCommon.hpp - Common functions in ONNXToHARX
//---------===//
//
...
// =============================================================================
//
// This file contains common methods used in ONNX to HARX lowering.
//
//===----------------------------------------------------------------------===//

#ifndef ONNX_MLIR_HARX_COMMON_H
#define ONNX_MLIR_HARX_COMMON_H
...
const std::string ARX_DEVICE = "nnpa";

// bool isEnableScalarBcastBinary();

struct OnnxToHARXLoweringConfiguration {
  static int optReportARXUnsupportedOps;
  static int reportOnARXUnsupportedOps;
};
...
void addDynamicallyLegalOpFor(mlir::ConversionTarget *target,
  ...
  target->addDynamicallyLegalOp<OP_TYPE>([dimAnalysis, checkLegalityFn](
    ...
    assert((!device ||
               (device &&
                   (device.getValue().equals_insensitive("") ||
                       ...
                       device.getValue().equals_insensitive(ARX_DEVICE)))) &&
           "Invalid device name");
    ...
    // If not CPU, check if the op is legal for ARX.
    bool isLegalForARX = false;
    if (checkLegalityFn)
      isLegalForARX = !checkLegalityFn(op, dimAnalysis);
    else {
      // Check zDNN limitations for each input tensors.
      // TODO: Check tensor size ARX_MAXIMUM_TENSOR_SIZE of another limitation
      bool exceedLimit =
            ...
            if (auto valueType =
              ...
              for (int64_t i = 0; i < valueRank; ++i) {
                ...
                if (!mlir::ShapedType::isDynamic(dim) &&
                    dim > ARXGetMaxForDim(i, valueRank))
                  return true;
              }
            }
            return false;
...
/// ARXGetMaxForDim and the last chuck can be smaller.
