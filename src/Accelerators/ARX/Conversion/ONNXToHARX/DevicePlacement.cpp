...
//===-------- DevicePlacement.cpp - Device Placement for ARX -------------===//
//
// Copyright 2023 The IBM Research Authors.
//
// =============================================================================
//
// This pass is to set device (CPU, or ARX) for each operation in ONNX level.
...
// CPU or ARX.
// - "ARX": the operation may run on ARX or CPU, and the final decision is
// made by the compiler. If `device=ARX` is the result of this device-placement
// pass, then it means the compiler thinks it is suitable for ARX.
...
struct DevicePlacementPass
  ...
  StringRef getDescription() const override {
    return "Device placement for ARX";
  }
  ...
  Option<bool> useMuchFasterWithStickOps{*this, "use-much-faster-wsu",
      llvm::cl::desc("Enable FasterOpsWithStickUnstick ARXPlacementHeuristic"),
      llvm::cl::init(false)};
  Option<bool> useFasterWithStickOps{*this, "use-faster-wsu",
      llvm::cl::desc("Enable FasterOpsWithStickUnstick ARXPlacementHeuristic"),
      llvm::cl::init(false)};
  Option<bool> useFasterOps{*this, "use-faster",
      llvm::cl::desc("Enable FasterOps ARXPlacementHeuristic"),
      llvm::cl::init(false)};
...
void DevicePlacementPass::runOnOperation() {
  ...
  // Disable reporting on ARX unsupported ops in this pass even if
  // `-opt-report=ARXUnsupportedOps` is specified..
  OnnxToHARXLoweringConfiguration::reportOnARXUnsupportedOps = 0;
  ...
  // Run patterns that converts ONNX to HARX with analysis mode to collect
  // operations that are not converted. Those non-converted ops are running on
  // the host instead of accelerator.
  // Keep the order of calling pass synced with RewriteONNXForHARX.cpp and
  // ONNXToHARX.cpp.
  ...
  // Call RewriteONNXForHARX pass.
  RewritePatternSet Patterns1(context);
  getRewriteONNXForHARXPatterns(Patterns1, &dimAnalysis);
  getRewriteONNXForHARXDynamicallyLegal(&target, &dimAnalysis);
  (void)applyAnalysisConversion(module, target, std::move(Patterns1),
      ConversionConfig{.legalizableOps = &legalizedOps1});

  // Call ONNXToHARX pass for lowering multiple ONNX ops at once to HARX.
  // E.g. `onnx.ReLu (onnx.Conv)` to HARX.Conv.
  RewritePatternSet Patterns2(context);
  getONNXToHARXMultipleOpPatterns(Patterns2);
  (void)applyAnalysisConversion(module, target, std::move(Patterns2),
      ConversionConfig{.legalizableOps = &legalizedOps2});

  // Call ONNXToHARX pass for lowering a single ONNX op to HARX.
  RewritePatternSet Patterns3(context);
  getONNXToHARXOneOpPatterns(Patterns3);
  ...
  if (placementHeuristic == QualifyingOps)
    PlaceAllLegalOpsOnARX(context, ops, cpuOps);
  else if (placementHeuristic == FasterOps)
    PlaceBeneficialOpsOnARX(context, ops, &dimAnalysis, cpuOps);
  else if (placementHeuristic == FasterOpsWSU)
    PlaceBeneficialOpsOnARXWithStickUnstick(
        context, module, ops, &dimAnalysis, cpuOps);
  else if (placementHeuristic == MuchFasterOpsWSU)
    PlaceBeneficialOpsOnARXWithStickUnstick(context, module, ops, &dimAnalysis,
        cpuOps, /*min factor*/ 3.0, /*significant CPU Factor*/ 2.0,
        /*significant ARX Factor*/ 8.0);
