...
std::vector<std::pair<int64_t, std::string>> inOutDims_;
...
void splitString(const char* input, std::vector<std::string>& tokens, char separator) {
...
void genSignatureFunction(ModuleOp &module,
  ...
  { // Fill the initializer with pointers to entry point constants.
    ...
    splitString(dynEntryPointName.c_str(), tokens, '_');
    std::string partName = tokens[1];
  ...
  // array of entry point names.
  {
    ...
    LLVM::LLVMFuncOp funcOp = create.llvm.func(
        std::string("omQueryEntryPoints") + partName, llvmFnType, /*createUniqueFunc=*/true);
  ...
  SmallVector<std::string, 2> funcNames = {
      std::string("omInputSignature") + partName , std::string("omOutputSignature") + partName};
...
void ConvertKrnlToLLVMPass::runOnOperation() {
  ...
  std::cout <<"(ConvertKrnlToLLVM) dynEntryPointName = " << dynEntryPointName.c_str() << std::endl;
  ...
  if (entryGlobalOps.size() >= 1) {
    ...
    std::cout << "inSigGlobalOps.data()->getSymName().str() = " << inSigGlobalOps.data()->getSymName().str() << std::endl;

//    genUserMainFile(module);
...
//  printf(" [populateKrnlToLLVMConversion]\n");
