...
extern std::string dynEntryPointName;
//extern std::vector<std::string> entryPointNames_;
...
//    printf("KrnlEntryPointOpLowering init\n");
  }
...
//    std::string
    std::string tmpStr;
    dynEntryPointName = "run_" + staticEntryPointFuncName.str();
//    if (singleEntryPoint)
//      dynEntryPointName = DEFAULT_DYN_ENTRY_POINT;

    size_t pos = dynEntryPointName.rfind('_');
    if(pos != std::string::npos) {
      dynEntryPointName = dynEntryPointName.substr(0, pos);
    }

    printf("(KrnlEntryPoint) staticEntryPointFuncName = %s\n", staticEntryPointFuncName);
    printf("(KrnlEntryPoint) dynEntryPointFuncName = %s\n", dynEntryPointName.c_str());
...
private:
  ...
  void recordEntryPointSignatures(ModuleOp &module,
    ...
    printf("== recordEntryPointSignatures ==\n");
    ...
    printf("entryVarName = %s, KRNL_ENTRY_POINT_ID = %d, entryGlobalOps.size = %d\n", entryVarName.c_str(), KRNL_ENTRY_POINT_ID, entryGlobalOps.size());
...
void populateLoweringKrnlEntryPointOpPattern(LLVMTypeConverter &typeConverter,
  ...
  printf(" [populateLoweringKrnlEntryPointOpPattern]\n");
