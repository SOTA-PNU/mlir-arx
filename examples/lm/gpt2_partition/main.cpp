...
extern "C" OMTensorList *run_p0_gpt2o3(OMTensorList *);
extern "C" OMTensorList *run_p1_gpt2o3(OMTensorList *);
...
//  int inputNum = 1;
  ...
  int64_t input_ids[] = {4342, 318, 617, 2420, 284, 37773, 1058, 18435, 2159};

  int64_t rank = 3;
  int64_t input_shape[] = {1, 1, static_cast<int64_t>(9)};
  ...
  // Compute outputs.
  OMTensorList *tensorListOut = run_p0_gpt2o3(tensorListIn);

  OMTensor *tensor_p0_out0 = omTensorListGetOmtByIndex(tensorListOut, 0);
  inputTensors[1] = tensor;
  inputTensors[0] = tensor_p0_out0;

  tensorListIn = omTensorListCreate(inputTensors, 2);
  tensorListOut = run_p1_gpt2o3(tensorListIn);
